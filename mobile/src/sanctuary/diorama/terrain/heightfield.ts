/**
 * Sculpted island terrain (ADR 0011). Pure, seeded, and deterministic --
 * the same module produces both the mesh-style vertex arrays and the
 * `heightAt(x, z)` sampler used for element/dressing placement, so nothing
 * ever floats or sinks. In the 2.5D diorama the sampler grounds every
 * placed sprite; the arrays stay available for terrain-derived shading.
 *
 ...
 *
 * No renderer imports; plain typed arrays only.
 */
...
/**
 * Terrain height at world (x, z). Continuous; used by element placement,
 * dressing scatter, and any terrain-derived shading alike.
 */
