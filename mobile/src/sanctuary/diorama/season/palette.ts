/**
 * Season -> diorama scene palette. The API ships a season word and a
 * `background_tone` word (fresh | warm | fading | still); this module maps
 * them to sky/fog/light colors. Pure lookup -- unit-testable, no renderer
 * imports (colors are hex strings consumed by the render layer).
 *
 * Art direction: Breath-of-the-Wild-flavored painterly atmosphere --
 ...
 *
 * The seasonal MATERIAL palettes (recoloring zone sprites) live in the
 * asset pipeline (scripts/sanctuary_assets/palette/) and arrive with the
 * sprite atlas milestone; this file only governs atmosphere.
 */
