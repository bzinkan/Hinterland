 ...
 * SanctuarySnapshotDto -> ScenePlan, with no renderer imports and no
 * randomness. The same snapshot always produces a deep-equal plan, on any
 * platform -- this is what makes the diorama deterministic across visits
 * and devices, and what lets the renderer be swapped without touching
 * product logic.
 *
 ...
 * convention the 2D tier dots and the sprite manifest's `tierMin` use.
 */
...
} from "@/src/sanctuary/diorama/placement/seededLayout";
import {
  scenePalette,
  type ScenePalette,
} from "@/src/sanctuary/diorama/season/palette";

export type PlacedElement = {
  element: SanctuaryElementDto;
  /** Atlas sprite when drawn; fallback kind -> render the typed shape. */
  sprite: SpriteResolution;
...
export function buildScenePlan(snapshot: SanctuarySnapshotDto): ScenePlan {
  ...
  const zones: ZonePlan[] = SANCTUARY_ZONE_ORDER.flatMap((zoneId) => {
    ...
    const placed: PlacedElement[] = zoneElements.map((element, index) => ({
      element,
      sprite: resolveElementSprite(element.icon, element.element_type),
